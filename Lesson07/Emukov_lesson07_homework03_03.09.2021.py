...
class Cell:
    ...
    def __add__(self, other):
        return f"Операция сложения клеток {self.quantity + other.quantity}"

    def __sub__(self, other):
        sub = self.quantity - other.quantity
        return f"Операция уменьшения клеток {sub}" if sub > 0 else "Клетки нету"

    def __mul__(self, other):
        return f"Операция перемножения клеток {self.quantity * other.quantity}"

    def __truediv__(self, other):
        return f"Операция деления клеток {self.quantity // other.quantity}"
